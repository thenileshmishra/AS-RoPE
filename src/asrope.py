...
Extends RoPE with a learnable per-(head, frequency) gate vector that scales
base rotary frequencies before computing phase:
...
class ASRotaryEmbedding(nn.Module):
    ...
    def _build_freqs_cis(
        self, seq_len: int, freq_gates: torch.Tensor, device: torch.device
    ) -> torch.Tensor:
        ...
        gates = freq_gates.view(self.n_heads, self.head_dim // 2).to(
    ...
    def apply_rotary(
        self, x: torch.Tensor, seq_len: int, freq_gates: torch.Tensor
    ) -> torch.Tensor:
