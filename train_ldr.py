...
def train(args: argparse.Namespace) -> None:
    ...
    for step in range(1, args.max_steps + 1):
        ...
        with torch.no_grad():
            pred_tokens = logits.argmax(dim=-1)
            query_positions = (target_ids != -100)
            train_acc = (pred_tokens[query_positions] == target_ids[query_positions]).float().mean().item()

        if step % args.log_interval == 0 or step == 1:
            elapsed = time.time() - start_time
            print(
                f"step={step:6d} loss={loss.item():.6f} "
                f"query_acc_512={train_acc * 100:.2f}% elapsed={elapsed:.1f}s"
            )

        if step >= args.min_steps and train_acc >= args.target_train_acc:
            print(
                f"Reached target accuracy at step {step}: "
                f"query_acc_512={train_acc * 100:.2f}%"
            )
            break
    ...
    print(f"checkpoint_saved={ckpt_path.resolve()}")

    print("\nEvaluation (query-position accuracy):")
    for eval_len in (512, 1024, 2048, 4096):
        acc = evaluate_query_accuracy(
            model=model,
            seq_len=eval_len,
        ...
        print(f"seq_len={eval_len:4d} query_acc={acc * 100:.2f}%")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target_train_acc", type=float, default=0.995)
